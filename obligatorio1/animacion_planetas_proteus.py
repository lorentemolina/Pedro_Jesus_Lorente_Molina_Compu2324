...
#file_in = "geocent.dat" # Nombre del fichero de datos
file_out = "heliocentrico" # Nombre del fichero de salida (sin extensión)
...
for i, (planet_pos, radius) in enumerate(zip(frames_data[0], planet_radius)):
    x, y = planet_pos
    color_planeta = colores[i % len(colores)]  # Usa el módulo para asegurarte de que obtienes un color válido
    planet_point = Circle((x, y), radius, color=color_planeta)
    ...
    # Crea manualmente los objetos de los planetas para la leyenda
    legend_handles = [plt.Circle((0, 0), 1, color=color) for color in colores]

    # Crea la leyenda
    ax.legend(legend_handles, planet_names, loc="upper right", fontsize="small")
